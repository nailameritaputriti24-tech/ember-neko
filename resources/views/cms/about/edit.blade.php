...
@section('content')
    <div class="mx-auto max-w-7xl">
        <div class="mb-6">
            ...
            <p class="mt-2 text-sm text-slate-600">Kelola gambar dan konten About untuk bahasa Indonesia dan Inggris.</p>
        </div>

        <form method="POST" action="{{ route('cms.about.update') }}" enctype="multipart/form-data" class="bg-white p-6 shadow-sm ring-1 ring-slate-200 sm:p-8">
            @csrf
            @method('PUT')
            ...
            <div class="grid gap-px bg-slate-200 lg:grid-cols-2">
                @foreach (['id' => ['Indonesia', 'ID'], 'en' => ['English', 'EN']] as $locale => [$languageName, $code])
                    ...
                    <section class="bg-white p-5 sm:p-6">
                        ...
                        @if ($about?->{$imageField})
                            <img src="{{ asset('storage/'.$about->{$imageField}) }}" alt="About {{ $code }}" class="mb-4 aspect-video w-full bg-slate-100 object-cover">
                        @endif
